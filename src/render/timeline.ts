...
export function renderTimeline(timeline: Timeline, root: HTMLElement): void {
  ...
  // Placed before the hops: it's about where the message came from rather
  // than how it travelled, and it's the thing a reader would want to know first.
  if (timeline.aiDeclarations.length > 0) root.append(renderAiDeclarations(timeline));
...
/**
 * Call out a sender's own claim that the message was AI-generated.
 *
 * The wording is deliberately about the sender's tooling rather than the
 * message itself. The header is trivially added or removed, so this can say
 * what the sender declared — never that a message is or isn't AI-written.
 */
function renderAiDeclarations(timeline: Timeline): HTMLElement {
  const box = el("section", "ai-flag");

  box.append(el("p", "ai-flag-title", "Marked as AI-generated by the sender"));

  const list = el("dl", "ai-flag-headers");
  for (const header of timeline.aiDeclarations) {
    list.append(el("dt", undefined, header.name), el("dd", undefined, header.value));
  }
  box.append(list);
  ...
  box.append(
    el(
      "p",
      "ai-flag-note",
      "This header is set by whatever software sent the message, and anything can add or strip it. " +
        "It shows what the sender's tooling declared, and its absence on other mail proves nothing."
    )
  );
